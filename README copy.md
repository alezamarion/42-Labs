...
Este projeto contém o desenvolvimento de uma aplicação de análise e compressão de dados utilizando o algoritmo de compressão Huffman que recebe multiplos dados que ao serem comprimidos devem ser descomprimidos e ter as informações coletadas para a análise do processo e dado.

### Algoritmo de Huffman:
O algoritmo de Huffman é um processo de compactação eficiente sem pecar de dados podendo ser adaptado para qualquer tipo de arquivo como imagem, vídeo, audio, porém é muito utilizado para compactação de texto devido sua características.
